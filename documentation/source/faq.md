...
- [The SIL Open Font License (OFL-FAQ)](https://scripts.sil.org/OFL-FAQ*web)
...
#### <a name="ME"></a>*How do I use Arabic script in InDesign ME? (obsolete?)*

You should follow the above instructions. However, some people have run into problems with the combining marks behaving improperly (farther away from the base character than the font design intended, or even above a character when it should be below). In that case, go to the Paragraph Stylesheet:
...
#### <a name="LOCorruption"></a>*I am experiencing what appears to be corruption of my data in LibreOffice when I use one of your fonts. What is the matter and how do I fix it? (obsolete?)*
...
Some of our fonts support [Graphite](http://graphite.sil.org/) rendering. See [this page](http://scripts.sil.org/cms/scripts/page.php?site*id=projects&amp;item*id=graphite*apps) for a list of all applications supporting Graphite rendering.
...
[International Components for Unicode (ICU)](http://www.icu-project.org/) includes a shaping engine for Arabic text and thus applications such as XeTeX 2012 (and earlier) that use ICU Layout will be able to render text using SIL's Arabic fonts. The ICU Layout library used in XeTeX (TeXLive 2012) has been updated to support the Arabic Supplement characters added in Unicode 5.1 (but not 6.1).

XeTeX (from TeXLive 2020 on) now supports the HarfBuzz shaper which supports Graphite as well as all Unicode characters through Unicode 13.0. 

The rendering engine in Adobe's InDesign CS6+ has also been updated to support the Arabic Supplement characters added in Unicode 5.1 (but not 6.0, 6.1, 7.0 or 8.0).
